...
public delegate void ParallaxControlElementRepositionLogicFunction(ParallaxControl p, ParallaxControlElement element, float amount, GameObject obj, Renderer r);
...
public class ParallaxControl : MonoBehaviour
{
    ...
    public List<ParallaxControlElement> Elements;
    ...
    {
        ParallaxControlElement e = Elements[i];
    ...
    public void AddElement(ParallaxControlElement e)
    {
        if (Elements == null)
        {
            Elements = new List<ParallaxControlElement>();
        }
    ...
    void Update()
    {
        ...
        foreach (ParallaxControlElement e in Elements)
        {
...
public enum ParallaxControlPositionMode
{
...
/// </summary>
[Serializable]
public class ParallaxControlElementRepositionLogic
{
    ...
    public ParallaxControlPositionMode PositionMode = ParallaxControlPositionMode.WrapAnchorBottom;
...
[Serializable]
public class ParallaxControlElement
{
    ...
    public ParallaxControlElementRepositionLogic RepositionLogic;

    [HideInInspector]
    public ParallaxControlElementRepositionLogicFunction RepositionLogicFunction;

    public void SetupState(ParallaxControl p, Camera c, int index)
    {
        // add a second object if we need one to wrap properly
        if (RepositionLogic.PositionMode != ParallaxControlPositionMode.IndividualStartOffScreen &&
            RepositionLogic.PositionMode != ParallaxControlPositionMode.IndividualStartOnScreen &&
            GameObjects.Count == 1)
        {
    ...
    public void SetupScale(ParallaxControl p, Camera c, int index)
    {
    ...
    public void SetupPosition(ParallaxControl p, Camera c, int index)
    {
        ...
        for (int i = 0; i < GameObjects.Count; i++)
        {
                ...
                RepositionLogic.PositionMode == ParallaxControlPositionMode.IndividualStartOnScreen)
            {
                ...
                if (p.IsHorizontal)
                {
                    x = (RepositionLogic.PositionMode == ParallaxControlPositionMode.IndividualStartOnScreen ? r.bounds.min.x : 0);
                    y = (RepositionLogic.PositionMode == ParallaxControlPositionMode.IndividualStartOnScreen ? r.bounds.min.y : start + r.bounds.size.y);
                }
                else
                {
                    x = (RepositionLogic.PositionMode == ParallaxControlPositionMode.IndividualStartOnScreen ? r.bounds.min.x : start + r.bounds.size.x);
                    y = (RepositionLogic.PositionMode == ParallaxControlPositionMode.IndividualStartOnScreen ? r.bounds.min.y : 0);
                }
                ParallaxControl.SetPosition(obj, r, x, y);
            }
            else
            {
                ...
                if (RepositionLogic.PositionMode == ParallaxControlPositionMode.WrapAnchorTop)
                {
                    if (p.IsHorizontal)
                    {
                        Vector3 topWorld = c.ViewportToWorldPoint(new Vector3(0.0f, 1.0f, 0.0f));
                        ParallaxControl.SetPosition(obj, r, offset, topWorld.y - r.bounds.size.y);
                    }
                    else
                    {
                        Vector3 topWorld = c.ViewportToWorldPoint(new Vector3(1.0f, 0.0f, 0.0f));
                        ParallaxControl.SetPosition(obj, r, topWorld.x - r.bounds.size.x, offset + r.bounds.size.y);
                    }
                }
                else if (RepositionLogic.PositionMode == ParallaxControlPositionMode.WrapAnchorBottom)
                {
                    if (p.IsHorizontal)
                    {
                        ParallaxControl.SetPosition(obj, r, offset, screenLeft.y);
                    }
                    else
                    {
                        ParallaxControl.SetPosition(obj, r, screenLeft.x, offset);
                    }
                }
                else
                {
                    ...
                    if (p.IsHorizontal)
                    {
                        ParallaxControl.SetPosition(obj, r, offset, r.bounds.min.y);
                    }
                    else
                    {
                        ParallaxControl.SetPosition(obj, r, r.bounds.min.x, offset);
                    }
                }
    ...
    public void Update(ParallaxControl p, float t, Camera c)
    {
        ...
        bool wrap = RepositionLogic.PositionMode != ParallaxControlPositionMode.IndividualStartOffScreen &&
            RepositionLogic.PositionMode != ParallaxControlPositionMode.IndividualStartOnScreen;
            ...
            else if (p.IsHorizontal)
            {
                if (t > 0 && (screenEdge.y >= c.rect.height || screenEdgeValue >= maxEdge))
                {
                    ...
                    else
                    {
                        ...
                        ParallaxControl.SetPosition(obj, r, leftEdge.x - newWorldPoint.x, newWorldPoint.y);
                    }
                }
                else if (t < 0 && (screenEdge.y >= c.rect.height || screenEdge.x < minEdge))
                {
                    ...
                    else
                    {
                        ...
                        float randY = UnityEngine.Random.Range(RepositionLogic.MinYPercent, RepositionLogic.MaxYPercent);
                        Vector3 newWorldPoint = c.ViewportToWorldPoint(new Vector3(randX, randY));
                        ParallaxControl.SetPosition(obj, r, rightEdge.x + newWorldPoint.x, newWorldPoint.y);
                    }
                }
            }
            else
            {
                if (t > 0 && (screenEdge.x >= c.rect.width || screenEdgeValue >= maxEdge))
                {
                    ...
                    else
                    {
                        ...
                        float randY = UnityEngine.Random.Range(RepositionLogic.MinYPercent, RepositionLogic.MaxYPercent);
                        Vector3 newWorldPoint = c.ViewportToWorldPoint(new Vector3(randX, randY));
                        ParallaxControl.SetPosition(obj, r, newWorldPoint.x, bottomEdge.y - newWorldPoint.y);
                    }
                }
                else if (t < 0 && (screenEdge.x >= c.rect.width || screenEdge.y < minEdge))
                {
                    ...
                    else
                    {
                        ...
                        float randY = UnityEngine.Random.Range(RepositionLogic.MinYPercent, RepositionLogic.MaxYPercent);
                        Vector3 newWorldPoint = c.ViewportToWorldPoint(new Vector3(randX, randY));
                        ParallaxControl.SetPosition(obj, r, newWorldPoint.x, topEdge.y + newWorldPoint.y);
                    }
                }
            }
        }
